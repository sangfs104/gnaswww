...
// This runs on the SERVER (build time / request time), not in the browser.
// `revalidate` here actually works: Next.js caches this response and only
// re-fetches from your API once every 60s, shared across ALL visitors.
...
// Optional but recommended: pre-render this page at build time and
// silently refresh it in the background every 60s (ISR), instead of
// hitting your API on every request.
export const revalidate = 60;

export default async function CollectionsPage() {
  const products = await getProducts();

  // Products arrive already rendered in the initial HTML.
  // No spinner, no client-side round trip, no waterfall.
  return <ProductCollection initialProducts={products} />;
}
