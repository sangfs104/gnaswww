...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Lưu token + user thống nhất
      setAuth(data.token, data.user);

      // Merge giỏ hàng guest → user (nếu có)
      const guestId = localStorage.getItem("guestId");
      if (guestId && data.user?.id) {
        try {
          ...
          // Không chặn login nếu merge lỗi
        }
      }

      // Báo hiệu toàn app cập nhật cart + auth
      window.dispatchEvent(new Event("cart-updated"));
      ...
      const redirect = searchParams.get("redirect") || "/";
      router.push(redirect);
      router.refresh(); // giúp server component cập nhật nếu cần
    } catch (err) {
      console.error(err);
      setError("Có lỗi xảy ra. Vui lòng thử lại.");
    } finally {
        ...
        <div className="bg-white border border-gray-200 rounded-3xl shadow-xl p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-black hover:bg-gray-900 text-white py-4 rounded-2xl font-semibold text-lg transition-all disabled:opacity-70 disabled:cursor-not-allowed mt-2"
            >
              {loading ? "Đang đăng nhập..." : "Đăng nhập"}
            </button>
          </form>
